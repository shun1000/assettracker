...
const illegalFakeCamera1: any = {
  deviceId: "10",
  groupId: "1",
  kind: "videoinput"
};

const legacyFakeCamera1: any = {
  groupId: "1",
  kind: "videoinput",
  label: "Fake Camera Device (back)"
};

const legacyFakeCamera2: any = {
  deviceId: "100",
  groupId: "1",
  kind: "video",
  label: "Fake Camera Device (front)"
...
test.beforeEach(() => {
  (<any>CameraAccess.cameras) = undefined;
  (<any>window).MediaStreamTrack = undefined;
  (<any>window.navigator).enumerateDevices = undefined;
});

test.serial("getCameras (errors)", async t => {
  resetStubs();
  let error: Error = await t.throws(CameraAccess.getCameras());
  t.is(error.name, "UnsupportedBrowserError");
  t.false(getUserMediaStub.called);
  t.false(getSourcesStub.called);
  fakeCompatibleBrowser();
  (<any>window.navigator).mediaDevices.getUserMedia = getUserMediaStub.rejects(new Error("Test error"));
  resetStubs();
...
test.serial("getCameras (MediaStreamTrack.getSources)", async t => {
  ...
  (<any>window).MediaStreamTrack = {
    getSources: getSourcesStub.callsArgWith(0, null)
  };
  resetStubs();
  const error: Error = await t.throws(CameraAccess.getCameras());
  t.is(error.name, "UnsupportedBrowserError");
  t.true(getUserMediaStub.called);
  ...
  resetStubs();
  const camerasCached: Camera[] = await CameraAccess.getCameras();
  t.false(getUserMediaStub.called);
  t.false(getSourcesStub.called);
...
test.serial("getCameras (navigator.mediaDevices.enumerateDevices)", async t => {
  fakeCompatibleBrowser();
  (<any>window.navigator.mediaDevices).enumerateDevices = enumerateDevicesStub.resolves([
    fakeCamera1,
    fakeCamera2,
    illegalFakeCamera1,
    fakeMicrophone
...
test.serial("getCameras (navigator.enumerateDevices)", async t => {
  fakeCompatibleBrowser();
  (<any>window.navigator).enumerateDevices = enumerateDevicesStub.resolves([
    fakeCamera1,
    fakeCamera2,
    illegalFakeCamera1,
    fakeMicrophone
...
test.serial("getCameras (internationalized label)", async t => {
  fakeCompatibleBrowser();
  (<any>window.navigator.mediaDevices).enumerateDevices = enumerateDevicesStub.resolves([fakeCamera3]);
  resetStubs();
  const cameras: Camera[] = await CameraAccess.getCameras();
  t.not(cameras, null);
...
test.serial("getCameras (no front/back label information)", async t => {
  fakeCompatibleBrowser();
  (<any>window.navigator.mediaDevices).enumerateDevices = enumerateDevicesStub.resolves([fakeCamera4, fakeCamera5]);
  resetStubs();
  const cameras: Camera[] = await CameraAccess.getCameras();
  t.not(cameras, null);
...
test.serial("accessCameraStream", async t => {
  fakeCompatibleBrowser();
  resetStubs();
  let mediaStream: MediaStream = await CameraAccess.accessCameraStream(0);
  t.true(getUserMediaStub.called);
